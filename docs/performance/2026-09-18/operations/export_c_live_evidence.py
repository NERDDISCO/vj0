...
metadata={'observed_epoch':time.time(),'original_dispatcher_state':Path('/proc/568/stat').read_text().rpartition(') ')[2].split()[0],'cgroup_files':{str(p):p.read_text() for p in [Path('/proc/self/cgroup'),Path('/sys/fs/cgroup/cpu.max'),Path('/sys/fs/cgroup/cpu.stat'),Path('/sys/fs/cgroup/cpu/cpu.cfs_quota_us'),Path('/sys/fs/cgroup/cpu/cpu.cfs_period_us'),Path('/sys/fs/cgroup/cpu/cpu.stat'),Path('/sys/fs/cgroup/cpu,cpuacct/cpu.cfs_quota_us'),Path('/sys/fs/cgroup/cpu,cpuacct/cpu.cfs_period_us'),Path('/sys/fs/cgroup/cpu,cpuacct/cpu.stat')] if p.is_file()},'gpu_snapshot':subprocess.check_output(['nvidia-smi','--query-gpu=index,uuid,name,driver_version,memory.used,utilization.gpu','--format=csv,noheader'],text=True,timeout=15),'compute_processes':subprocess.check_output(['nvidia-smi','--query-compute-apps=pid,process_name','--format=csv,noheader'],text=True,timeout=15)}
