...
(() => {
  ...
  RTCPeerConnection.prototype.createDataChannel = function(...args) {
    ...
    channel.send = data => {
      ...
      record('sent',{id,captureAt:value.capture,bytes:tagged.byteLength,buffered:channel.bufferedAmount,audioLevel:value.audioLevel});
      return result;
    };
    return channel;
  };
...
(() => {
  ...
  const channels=[],acks=new Map();let recording=false,samples=[],checks={},impulses=[],pulseTimers=[];
  const epoch=()=>performance.timeOrigin+performance.now();
  ...
  const api={thresholdBytes,
    ...
    begin(){stopImpulses();samples=[];checks={};impulses=[];recording=true;},
    ...
    snapshot(){return {thresholdBytes,channels:channelState(),checks:structuredClone(checks),
      samples:samples.slice(),impulses:impulses.slice()};},
