...
# Resolve the companion from this worker's own source directory. Benchmarks also
# import this file by path, without putting that directory on Python's sys.path.
_runtime_spec = importlib.util.spec_from_file_location(
    "vj0_worker_runtime", Path(__file__).with_name("worker_runtime.py"))
_runtime = importlib.util.module_from_spec(_runtime_spec)
_runtime_spec.loader.exec_module(_runtime)
configure_torch_threads = _runtime.configure_torch_threads
configure_terminal_noop = _runtime.configure_terminal_noop
configure_gpu_output_cast = _runtime.configure_gpu_output_cast
...
def setup_pipeline():
    # Apply before model loading or compiled/eager CPU work. Host CPU count can
    # greatly exceed a pod's quota; oversized intra-op pools waste that quota.
    configure_torch_threads(torch, log)
    ...
    # Optional, source-pinned prediction skip. The existing generate() schedule,
    # callbacks and main-thread warmup still execute without changing settings.
    configure_terminal_noop(pipe, log)
    configure_gpu_output_cast(pipe, np, log)
    return pipe
