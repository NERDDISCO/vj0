...
def code_cache_snapshot():
    from torch._inductor.codecache import PyCodeCache
    if not isinstance(PyCodeCache.cache, dict):
        raise RuntimeError('Unrecognized PyCodeCache API; cannot attest generated source')
    return {key: id(module) for key, module in PyCodeCache.cache.items()}


def archive_generated_sources(before, output, tag):
    """Save only wrappers loaded by this candidate compilation, not old caches."""
    from torch._inductor.codecache import PyCodeCache
    sources, errors = [], []
    changed = [(key, module) for key, module in PyCodeCache.cache.items() if before.get(key) != id(module)]
    if not changed or len(changed) > 64:
        return [], ['No newly bound candidate wrappers or more than 64 wrappers; source proof ambiguous']
    for index, (key, module) in enumerate(changed):
        record = {'cache_key': str(key), 'original_path': getattr(module, '__file__', None)}
        try:
            path = Path(module.__file__)
            if path.stat().st_size > 16 * 1024 * 1024:
                raise ValueError('Generated wrapper exceeds bounded 16 MiB source audit')
...
def prove_template_execution(sources, trace, expected, compile_log, targets, *, micro=False, source_errors=()):
    ...
    return {'status': 'passed' if not errors else 'failed', 'errors': errors,
            ...
            'scope': 'Actual template GEMM events joined to newly loaded PyCodeCache source, FP8 signature, tl.dot, static shape and allowlisted tile; pointwise/reduction kernels never count'}
            ...
            for index,shape in enumerate(targets):
                ...
                try:
                    ...
                    with deadline(args.compile_timeout,'N03 one scaled-mm subgraph'):
                        ...
                        torch.cuda.synchronize()
                        source_before=code_cache_snapshot()
                    ...
                    sources,source_errors=archive_generated_sources(source_before,args.output,f'micro-{index}')
                ...
                for width,height in parse_sizes(args.sizes):
                    ...
                    with deadline(args.compile_timeout,f'N03 selected-shape transformer {width}x{height}'):
                        for variant in ['baseline','candidate']:
                            select(variant)
                            if variant=='candidate':source_before=code_cache_snapshot()
                            for _ in range(4):experiment.run(raw,width,height)
                    select('candidate')
                    ...
                    sources,source_errors=archive_generated_sources(source_before,args.output,tag)
