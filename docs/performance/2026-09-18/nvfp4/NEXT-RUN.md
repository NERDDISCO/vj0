# Executed full-model protocol


Completed as `model-02`; see [measured results](MODEL-RESULTS.md). The commands
below preserve the original pre-run instructions and are not a pending GPU task.
Existing output directories are intentionally protected from reuse.
